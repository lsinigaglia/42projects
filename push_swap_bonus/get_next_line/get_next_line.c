...
char	*get_the_line(int fd, char *raw_string)
{
	...
	while (!ft_strchr1(raw_string, '\n') && buff_lenght != 0)
	{
		...
		buf[buff_lenght] = '\0';
		raw_string = ft_strjoin1(raw_string, buf);
	}
	free(buf);
	return (raw_string);
}
...
char	*stop_at_newline(char *raw_string)
{
	...
	if (!raw_string[i])
		return (NULL);
	if (!ft_strchr1(raw_string, '\n'))
		return (ft_strdup1(raw_string));
...
char	*extract(char *raw_string)
{
	...
	i++;
	backup = malloc(ft_strlen1(raw_string + i) + 1);
	if (!backup)
		return (NULL);
	while (raw_string[i])
